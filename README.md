...
## 반응형 구축시 CSS _Calc()_ 활용팁

> 부트스트랩이나 반응형웹을 쉽게 구현할 수 있도록 쓰는 라이브러리등을 사용한다면 크게 고민하지 않아도 되지만, 프로젝트 할 때 처음부터 그런 라이브러리를 사용하지 않는다면 반응형에 대해 큰 어려움이 있을 것이다. 그러니 반응형 프로젝트를 하게 될 경우 유연하게 화면을 구상하려면 <b>고정된 단위인 px,pt가 아닌 em, %를 사용해야 한다 !</b> JS로도 제어가 가능하다고 하지만 CSS로 제어하는 것이 훨씬 쉽고 유지보수에도 좋다.
...
> 전체 너비 100%에서 50px을 제외한 길이만큼 영역 지정을 하고싶다. 이런경우 위처럼 간단하게 calc 사칙연산을 이용해 코딩해주면 된다. <b>고정된 영역에서 50px을 제외한 길이만큼 영역 지정하고 싶은 경우는 정말 간단하지만,반응형웹에서는 상대적인 단위를 사용하기 때문에 %,em등의 수치를 이용</b> 해야 한다. 브라우저 크기가 줄어들고 늘어날 때 마다 객체 영역의 크기가 망가지지 않고 유연하게 동작되게 하려면 꼭 필요하다.<br>
> width 속성 뿐 아니라 height, padding 등등의 모든 css가 선언된 곳에 calc를 사용할 수 있다. <b>💛 이처럼 반응형 웹을 구현할 경우 브라우저의 크기가 늘어나거나 줄어들 경우 일정 영역만큼을 뺀 나머지를 반환해 적용하는 방법이 최선책이라 할 수 있다. 💛</b>
...
> wrap클래스로 둘러싸인 3개의 div 태그들을 <b>정확히 3등분</b> 하기 위해서는 이런 방식으로 계산해주면된다.
...
> 전체 너비에서 중간에 60px만 띄우고 싶을때, 전체너비 100%인 반절 50%에서 60px반절 30px 빼주기!
...
> min(), max() 너비, 높이값 잡아줄 때 사용법

쉼표(,)에 의해 구분되는 min, max함수도 flexible한 글꼴 처리나 responsive layout에 유용하게 사용된다.
